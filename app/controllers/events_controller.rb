class EventsController < ApplicationController
  ...
  def show
    ...
    @markers = [{
        lat: @event.latitude,
        lng: @event.longitude,
        infoWindow: render_to_string(partial: "/shared/map_box", locals: { event: @event }),
        image_url: 'https://res.cloudinary.com/deduga1kr/image/upload/v1550070613/markers.png'
      }]
  end
  ...
  def event_params
    params.require(:event).permit(:title, :description, :date, :intensity, :price, :address, :photo, :photo_cache, :capacity)
  end
